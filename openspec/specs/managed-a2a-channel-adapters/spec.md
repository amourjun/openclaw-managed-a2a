# managed-a2a-channel-adapters Specification

## Purpose
Define the channel/domain adapter SPI above the canonical managed A2A core contract so channel-native requests can be normalized, diagnosed, and executed without coupling core behavior to any single IM platform.
## Requirements
...
#### Scenario: Additional reference wrapper is added without changing core semantics
- **WHEN** another channel-facing wrapper such as Telegram is introduced above the same SPI
- **THEN** the wrapper MUST normalize into the canonical managed request envelope without redefining core transport, policy, or audit semantics
- **AND** the plugin identity MUST remain channel-agnostic
